...
match — we will not publish details of an unpatched upstream issue.
...
- Theoretical query-DSL malice. fnd is a local tool whose queries
  ...
  query complexity (S4) as a hygiene exercise.
- Other-user-on-same-Mac escalation that depends on a setuid bug or
  ...
  state directory to 0o700 (M7) so a different mechanism would be
