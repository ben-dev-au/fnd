...
sidecar. The runtime gates in :func:`acorn.index._ensure_index` and
:func:`acorn.query._open_index` raise a clear error, but the user has to
...
_SIDECAR_NAME = ".acorn-schema-version"
    ...
    *,
    ...
    if status is SchemaStatus.EMPTY:
        typer.echo(
            ...
            f"`acorn collection add <name> --source <path>` "
            f"then run `acorn collection reindex <name>`.",
            err=True,
        )
    ...
    if is_tty is None:
        ...
        interactive = True if os.environ.get("ACORN_FORCE_TTY") == "1" else sys.stdin.isatty()
    else:
        interactive = is_tty

    if not interactive:
        typer.echo(
            "Re-run with `acorn collection reindex <name> --rebuild` "
    ...
    if not config.collections:
        typer.echo(
            "no collections configured. Run `acorn collection add <name> "
            "--source <path>` then `acorn collection reindex <name>`.",
            err=True,
        )
        raise typer.Exit(code=1)
