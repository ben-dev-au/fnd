...
class AcornProgressBar(Widget):
    ...
    AcornProgressBar.-idle { visibility: hidden; }
    AcornProgressBar > #progress_phase {
        width: auto;
        min-width: 16;
        color: $text-muted;
        padding: 0 1 0 0;
    }
    AcornProgressBar > Horizontal#progress_bar_wrap {
        width: 1fr;
        height: 1;
    }
    AcornProgressBar Bar > .bar--bar           { color: $accent; }
    AcornProgressBar Bar > .bar--indeterminate { color: $accent; }
    AcornProgressBar Bar > .bar--complete      { color: $success; }
    """

    def __init__(self) -> None:
        super().__init__(id="acorn_progress", classes="-idle")
    ...
    def set_total(self, total: int) -> None:
        with contextlib.suppress(Exception):
            bar = self.query_one("#acorn_progress_bar", ProgressBar)
            bar.update(total=max(1, total), progress=min(bar.progress, max(1, total)))

    def set_progress(self, progress: int) -> None:
        with contextlib.suppress(Exception):
            self.query_one("#acorn_progress_bar", ProgressBar).update(progress=progress)

    def reset(self) -> None:
        with contextlib.suppress(Exception):
            self.query_one("#acorn_progress_bar", ProgressBar).update(total=1, progress=0)
...
class ProgressFacility:
    ...
    def _widget(self) -> AcornProgressBar | None:
        with contextlib.suppress(Exception):
            return self._app.query_one(AcornProgressBar)
        return None
