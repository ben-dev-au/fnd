...
from fnd.tag_catalog import TagCount, tag_catalog
...
def test_lists_tags_per_source(tmp_path: Path) -> None:
    ...
    got = tag_catalog(index, collections=["vault"])
...
def test_parent_count_is_a_union_not_a_sum(tmp_path: Path) -> None:
    ...
    counts = {t.value: t.files for t in tag_catalog(index, collections=["vault"])["frontmatter"]}
...
def test_scoped_to_active_collections(tmp_path: Path) -> None:
    ...
    got_all = tag_catalog(index, collections=["vault", "work"])
...
def test_no_collection_scope_covers_everything(tmp_path: Path) -> None:
    ...
    got = tag_catalog(index, collections=[])
...
def test_sorted_by_count_then_name(tmp_path: Path) -> None:
    ...
    got = [t.value for t in tag_catalog(index, collections=["vault"])["frontmatter"]]
    assert got[0] == "common"
...
def test_sources_filter_limits_the_query(tmp_path: Path) -> None:
    ...
    got = tag_catalog(index, collections=["vault"], sources=["frontmatter"])
